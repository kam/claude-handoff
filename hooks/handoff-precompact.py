...
WRAPPERS = ("<command-name>", "<command-message>", "<local-command",
...
def scan(transcript):
    prompts, files, last_assistant = [], [], ("", 0)
    ...
    with open(transcript, encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if rec.get("type") == "user":
                ...
                if t and not t.startswith(WRAPPERS):
                    turn += 1
                    prompts.append(t)
...
def main():
    ...
    lines = [
        "---", "status: auto-snapshot", f"branch: {branch}",
        f"project: {os.path.basename(top)}", f"created: {now:%Y-%m-%dT%H:%M}",
    ...
    if want_prompts:
        ...
        lines += ["", f"## Last {len(shown)} of {len(prompts)} user prompts (verbatim, clipped, redacted)", ""]
    ...
    lines += ["", f"## Last text-bearing assistant message (after user prompt {last_turn} of {len(prompts)}; clipped)",
